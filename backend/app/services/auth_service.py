...
async def create_session(
    db: AsyncSession, agent: Agent, ip_address: str | None = None, user_agent: str | None = None,
) -> Session:
...
async def register_user(
    db: AsyncSession, email: str, password: str, name: str, workspace_name: str,
...
async def authenticate_user(
    ...
    if agent is None or agent.password_hash is None or not verify_password(password, agent.password_hash):
        if (settings.ADMIN_EMAIL and settings.ADMIN_PASSWORD
                and email == settings.ADMIN_EMAIL and password == settings.ADMIN_PASSWORD and agent is not None):
...
async def google_oauth_callback(
    ...
    if agent is None:
        ...
        db.add(workspace)
        await db.flush()
        agent = Agent(workspace_id=workspace.id, email=email, name=google_user["name"],
                      avatar_url=google_user.get("picture"), google_id=google_user.get("sub"))
        db.add(agent)
        await db.flush()
        membership = WorkspaceMembership(agent_id=agent.id, workspace_id=workspace.id, role="owner")
        db.add(membership)
