...
_COMMON_TIMEZONES = [
    ...
    "Asia/Dubai", "Asia/Riyadh", "Asia/Kuwait", "Asia/Karachi",
...
async def settings_general(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return request.app.state.templates.TemplateResponse("settings/index.html", {
        "request": request, "workspace": workspace, "timezones": tzs,
    })


@router.post("")
async def update_general(request: Request, name: str = Form(None), timezone: str = Form(None), db: AsyncSession = Depends(get_db)):
    workspace = request.state.workspace
...
async def export_data(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": "Export started. You'll be able to download it shortly.", "type": "success",
        })
    return RedirectResponse("/settings", status_code=303)
...
async def team_page(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return request.app.state.templates.TemplateResponse("settings/team.html", {
        "request": request, "members": members, "invites": invites,
    })


@router.post("/team/invite")
async def send_invite(request: Request, email: str = Form(...), role: str = Form("member"), db: AsyncSession = Depends(get_db)):
    workspace = request.state.workspace
    import secrets
    ...
    await db.flush()
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/invite_row.html", {
...
async def revoke_invite(request: Request, invite_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    workspace = request.state.workspace
    await db.execute(delete(WorkspaceInvite).where(WorkspaceInvite.id == invite_id, WorkspaceInvite.workspace_id == workspace.id))
    return HTMLResponse("")
...
async def billing_page(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return request.app.state.templates.TemplateResponse("settings/billing.html", {
        "request": request, "cloud_mode": cloud_mode, "balance": balance, "history": history, "workspace": workspace,
    })
...
async def update_auto_recharge(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    await db.execute(sa_update(Workspace).where(Workspace.id == workspace.id).values(
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": "Auto-recharge settings saved", "type": "success",
        })
...
async def integrations_page(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return request.app.state.templates.TemplateResponse("settings/integrations.html", {
        "request": request, "configs": configs,
    })


_VALID_INTEGRATION_TYPES = {
    "slack", "slack_bot", "email", "hubspot", "jira", "linear",
    "notion", "whatsapp", "messenger", "instagram", "shopify",
    "google_drive", "zendesk", "stripe", "salesforce", "dropbox",
}

_SENSITIVE_FIELDS = {"webhook_url", "api_key", "access_token", "api_token", "secret_key", "password", "page_access_token"}
...
async def save_integration(request: Request, integration_type: str, db: AsyncSession = Depends(get_db)):
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": f"{integration_type.replace('_', ' ').title()} settings saved", "type": "success",
        })
...
async def disconnect_integration(request: Request, integration_type: str, db: AsyncSession = Depends(get_db)):
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": f"{integration_type.replace('_', ' ').title()} disconnected", "type": "success",
        })
    return RedirectResponse("/settings/integrations", status_code=303)
...
async def save_llm_key(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if request.headers.get("HX-Request"):
        ...
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": msg, "type": "success" if key else "error",
        })
    return RedirectResponse("/settings/llm", status_code=303)
...
async def remove_llm_key(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": "API key removed", "type": "success",
        })
    return RedirectResponse("/settings/llm", status_code=303)
...
async def save_base_url(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": "Base URL saved", "type": "success",
        })
    return RedirectResponse("/settings/llm", status_code=303)
...
async def load_openrouter_models(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if not api_key:
        return request.app.state.templates.TemplateResponse("settings/_llm_models.html", {
            "request": request, "error": "No API key configured. Save your key first.", "models": [], "selected_models": [],
        })

    raw_base = workspace.openrouter_base_url or getattr(app_settings, "AI_BASE_URL", None) or "https://openrouter.ai/api/v1"
    from urllib.parse import urlparse
    parsed = urlparse(raw_base)
    if parsed.scheme not in ("http", "https"):
        return request.app.state.templates.TemplateResponse("settings/_llm_models.html", {
            "request": request, "error": "Invalid base URL scheme.", "models": [], "selected_models": [],
        })
...
async def save_llm_models(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        allowed = []
    await db.execute(
        sa_update(Workspace).where(Workspace.id == workspace.id).values(allowed_models=allowed)
    )
    # Keep in-memory object in sync for the response template
    workspace.allowed_models = allowed
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("settings/_llm_models_saved.html", {
...
async def save_default_chatbot_model(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": "Default chatbot model saved", "type": "success",
        })
    return RedirectResponse("/settings/llm", status_code=303)
...
async def save_internal_model(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": "Background tasks model saved", "type": "success",
        })
    return RedirectResponse("/settings/llm", status_code=303)
...
async def security_page(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return request.app.state.templates.TemplateResponse("settings/security.html", {
        "request": request, "twofa_enabled": user.two_fa_enabled, "setup_uri": None, "setup_secret": None,
    })
...
async def verify_2fa(request: Request, code: str = Form(...), db: AsyncSession = Depends(get_db)):
    ...
    if totp.verify(code):
        ...
        if request.headers.get("HX-Request"):
            return request.app.state.templates.TemplateResponse("components/toast.html", {
                "request": request, "message": "2FA enabled successfully", "type": "success",
            })
    else:
        if request.headers.get("HX-Request"):
            return request.app.state.templates.TemplateResponse("components/toast.html", {
                "request": request, "message": "Invalid code", "type": "error",
            })
...
async def disable_2fa(request: Request, code: str = Form(...), db: AsyncSession = Depends(get_db)):
    ...
    if user.totp_secret and pyotp.TOTP(user.totp_secret).verify(code):
        ...
        if request.headers.get("HX-Request"):
            return request.app.state.templates.TemplateResponse("components/toast.html", {
                "request": request, "message": "2FA disabled", "type": "success",
            })
    else:
        if request.headers.get("HX-Request"):
            return request.app.state.templates.TemplateResponse("components/toast.html", {
...
async def data_retention_page(request: Request, db: AsyncSession = Depends(get_db)):
    workspace = request.state.workspace
    return request.app.state.templates.TemplateResponse("settings/data_retention.html", {
...
async def update_data_retention(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": "Retention policy saved", "type": "success",
        })
...
async def webhooks_page(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    webhooks = (await db.execute(
        select(WorkspaceWebhook).where(WorkspaceWebhook.workspace_id == workspace.id)
    )).scalars().all()
    return request.app.state.templates.TemplateResponse("settings/webhooks.html", {
...
async def create_webhook(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    await db.flush()
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/webhook_row.html", {
...
async def update_webhook(request: Request, webhook_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(WorkspaceWebhook).where(WorkspaceWebhook.id == webhook_id)
    )
...
async def webhook_deliveries_partial(request: Request, webhook_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    deliveries = (await db.execute(
        select(WebhookDelivery).where(
            WebhookDelivery.webhook_id == webhook_id, WebhookDelivery.workspace_id == workspace.id,
        ).order_by(WebhookDelivery.created_at.desc()).limit(20)
    )).scalars().all()
    return request.app.state.templates.TemplateResponse("settings/_webhook_deliveries_partial.html", {
...
async def webhook_deliveries(request: Request, webhook_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    return request.app.state.templates.TemplateResponse("settings/webhook_deliveries.html", {
        "request": request, "deliveries": deliveries, "webhook_id": webhook_id,
    })
...
async def intelligence_config_page(request: Request, db: AsyncSession = Depends(get_db)):
    workspace = request.state.workspace
    return request.app.state.templates.TemplateResponse("settings/intelligence_config.html", {
        "request": request, "intelligence_config": workspace.intelligence_config or {},
    })
...
async def toggle_intelligence(request: Request, key: str, db: AsyncSession = Depends(get_db)):
    ...
    await db.execute(
        sa_update(Workspace).where(Workspace.id == workspace.id).values(intelligence_config=config)
    )
    if request.headers.get("HX-Request"):
        task_meta = {
            "auto_analyze": {"name": "Conversation Analysis", "desc": "Automatically analyze conversations for sentiment, intent, and topics when they close.", "schedule": "On conversation close"},
            "sentiment_trends": {"name": "Sentiment Trends", "desc": "Compute daily sentiment averages across all conversations.", "schedule": "Daily at 4:00 AM"},
            "gap_clustering": {"name": "Gap Clustering", "desc": "Cluster unanswered questions into topic groups using BERTopic.", "schedule": "Daily at 3:00 AM"},
        }
...
async def update_reports(request: Request, report_frequency: str = Form("off"), report_recipients: str = Form(""), db: AsyncSession = Depends(get_db)):
    workspace = request.state.workspace
    from sqlalchemy import update as sa_update
    config = dict(workspace.intelligence_config or {})
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": "Report settings saved", "type": "success",
        })
    return RedirectResponse("/settings/intelligence", status_code=303)
