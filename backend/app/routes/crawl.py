...
async def start_crawl(
    ...
    job_id, kb_id = await prepare_crawl(db, workspace.id, url, include_paths=include_paths,
                                          exclude_paths=exclude_paths, chatbot_id=chatbot_id)
    from app.background.runner import submit_job
    await submit_job("crawl_website", {"job_id": job_id})

    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/crawl_progress.html", {
            "request": request, "chatbot_id": chatbot_id, "job_id": job_id, "status": "pending",
        })
...
async def preview_crawl(
    ...
    try:
        ...
        return request.app.state.templates.TemplateResponse("components/crawl_preview.html", {
            "request": request, "error": str(exc), "url": url,
        })

    if not discovered:
        return request.app.state.templates.TemplateResponse("components/crawl_preview.html", {
            "request": request, "error": "No pages found. Check the URL and try again.", "url": url,
        })
    ...
    for path, count in sorted(path_counts.items()):
        ...
        if not parts:
            ...
            key = "/" + parts[0]
            parent = None if len(parts) <= 1 else key
    ...
    return request.app.state.templates.TemplateResponse("components/crawl_preview.html", {
        "request": request, "url": url, "chatbot_id": chatbot_id,
        "total": len(discovered), "groups": level1,
    })
...
async def retry_failed(request: Request, chatbot_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    if kbs:
        docs = (await db.execute(
            select(Document).where(Document.knowledge_base_id.in_([kb.id for kb in kbs]), Document.status == "failed")
        )).scalars().all()
        for doc in docs:
            doc.status = "pending"
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": f"Retrying {count} failed documents", "type": "success",
        })
    return RedirectResponse(f"/chatbots/{chatbot_id}/sources", status_code=303)
...
async def crawl_status(request: Request, chatbot_id: uuid.UUID, job_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    if not job:
        return HTMLResponse("Not found", status_code=404)
    return request.app.state.templates.TemplateResponse("components/crawl_progress.html", {
        "request": request, "chatbot_id": chatbot_id, "job_id": str(job.id),
        "status": job.status, "pages_discovered": job.pages_discovered,
        "pages_queued": job.pages_queued, "pages_failed": job.pages_failed,
