...
async def dashboard(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    total_convs = (await db.execute(
        select(func.count(Conversation.id)).where(Conversation.workspace_id == ws_id)
    )).scalar_one()

    # Conversations 30d
    convs_30d = (await db.execute(
        select(func.count(Conversation.id)).where(
            Conversation.workspace_id == ws_id, Conversation.created_at >= thirty_days_ago,
        )
    )).scalar_one()
    ...
    resolved = (await db.execute(
        select(func.count(Conversation.id)).where(
            Conversation.workspace_id == ws_id, Conversation.autonomous_resolved == True,
        )
    )).scalar_one()
    ...
    thumbs_up = (await db.execute(
        select(func.count(MessageFeedback.id)).where(
            MessageFeedback.workspace_id == ws_id, MessageFeedback.rating == "thumbs_up",
        )
    )).scalar_one()
    thumbs_down = (await db.execute(
        select(func.count(MessageFeedback.id)).where(
            MessageFeedback.workspace_id == ws_id, MessageFeedback.rating == "thumbs_down",
        )
    )).scalar_one()
    ...
    avg_sentiment = (await db.execute(
        select(func.avg(ConversationAnalysis.sentiment_score)).where(
            ConversationAnalysis.workspace_id == ws_id, ConversationAnalysis.sentiment_score.isnot(None),
        )
    )).scalar_one()
    ...
    topics_result = await db.execute(
        ...
        .where(ConversationAnalysis.workspace_id == ws_id)
    )
    topic_counts = {}
    ...
    gaps = (await db.execute(
        select(GapCluster).where(GapCluster.workspace_id == ws_id, GapCluster.status == "open")
