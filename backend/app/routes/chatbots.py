...
async def list_chatbots(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    for bot in chatbots:
        ...
        total = (
            await db.execute(select(func.count(Conversation.id)).where(Conversation.chatbot_id == bot.id))
        ).scalar_one()
...
async def chatbot_setup(request: Request, chatbot_id: uuid.UUID, step: int = 0, db: AsyncSession = Depends(get_db)):
    workspace = request.state.workspace
    chatbot = (
        await db.execute(select(Chatbot).where(Chatbot.id == chatbot_id, Chatbot.workspace_id == workspace.id))
    ).scalar_one_or_none()
    if not chatbot:
    ...
    if chatbot.active_crawl_job_id:
        ...
        crawl_job = (
            await db.execute(select(CrawlJob).where(CrawlJob.id == chatbot.active_crawl_job_id))
        ).scalar_one_or_none()
...
async def chatbot_detail(request: Request, chatbot_id: uuid.UUID, days: int = 30, db: AsyncSession = Depends(get_db)):
    ...
    thumbs_up = (
        await db.execute(
            select(func.count()).select_from(feedback_filter.where(MessageFeedback.rating == "thumbs_up").subquery())
        )
    ).scalar_one()
    thumbs_down = (
        await db.execute(
            select(func.count()).select_from(feedback_filter.where(MessageFeedback.rating == "thumbs_down").subquery())
        )
    ).scalar_one()
    ...
    gaps = (
        (
            await db.execute(
                select(GapCluster)
                .where(
                    GapCluster.workspace_id == workspace.id,
                    GapCluster.chatbot_id == chatbot_id,
...
async def chatbot_sources(request: Request, chatbot_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    if kbs:
        ...
        doc_result = await db.execute(
            select(Document).where(Document.knowledge_base_id.in_(kb_ids)).order_by(Document.created_at.desc())
        )
    ...
    if kbs:
        crawl_result = await db.execute(
            select(CrawlJob).where(CrawlJob.kb_id.in_([kb.id for kb in kbs])).order_by(CrawlJob.created_at.desc())
        )
        crawl_jobs = crawl_result.scalars().all()

    return request.app.state.templates.TemplateResponse(
        "chatbots/sources.html",
        {
            "request": request,
            "chatbot": chatbot,
            "documents": docs,
