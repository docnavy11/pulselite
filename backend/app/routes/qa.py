...
async def qa_list(
    request: Request, chatbot_id: uuid.UUID, status: str | None = None, db: AsyncSession = Depends(get_db)
):
...
async def run_all_tests(request: Request, chatbot_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    await db.execute(
        update(QAPair)
        .where(QAPair.chatbot_id == chatbot_id, QAPair.workspace_id == workspace.id)
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse(
            ...
            {
                "request": request,
                "message": f"Queued {len(pairs)} Q&A pairs for testing",
                "type": "success",
            },
        )
    return RedirectResponse(f"/chatbots/{chatbot_id}/qa", status_code=303)
