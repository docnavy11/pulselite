...
from app.services.auth_service import authenticate_user, delete_session, google_oauth_callback, register_user
...
async def login(
    ...
    response.set_cookie(
        "session_id", session.id, httponly=True, samesite="lax", max_age=settings.SESSION_EXPIRY_DAYS * 86400
    )
    return response
...
async def register(
    ...
    password: str = Form(...),
    name: str = Form(...),
    workspace_name: str = Form("My Workspace"),
    db: AsyncSession = Depends(get_db),
):
    try:
...
async def switch_workspace(
    ...
    await db.execute(sa_update(Session).where(Session.id == session_id).values(workspace_id=workspace_id))
    await db.commit()
...
async def create_workspace(
    ...
    session_id = request.cookies.get("session_id")
    await db.execute(sa_update(Session).where(Session.id == session_id).values(workspace_id=new_ws.id))
    await db.commit()
