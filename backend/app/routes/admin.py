...
async def admin_overview(
    ...
    if tab == "health":
        ...
        ctx["chunk_count"] = (
            await db.execute(select(func.count(Chunk.id)).where(Chunk.workspace_id == workspace.id))
        ).scalar_one()
        ctx["conv_count"] = (
            await db.execute(select(func.count(Conversation.id)).where(Conversation.workspace_id == workspace.id))
        ).scalar_one()
        ctx["chatbot_count"] = (
            await db.execute(select(func.count(Chatbot.id)).where(Chatbot.workspace_id == workspace.id))
        ).scalar_one()
        ctx["kb_count"] = (
            await db.execute(select(func.count(KnowledgeBase.id)).where(KnowledgeBase.workspace_id == workspace.id))
        ).scalar_one()
        ...
        limit = 50
        total = (
            await db.execute(select(func.count(AuditLog.id)).where(AuditLog.workspace_id == workspace.id))
        ).scalar_one()
        audit_logs = (
            (
                await db.execute(
        ...
        limit = 50
        total = (
            await db.execute(select(func.count(RetrievalLog.id)).where(RetrievalLog.workspace_id == workspace.id))
        ).scalar_one()
        rows = (
            await db.execute(
