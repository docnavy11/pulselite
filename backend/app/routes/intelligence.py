...
async def gap_list(request: Request, status: str | None = None, db: AsyncSession = Depends(get_db)):
    ...
    clusters = (await db.execute(query.order_by(GapCluster.gap_count.desc()).limit(50))).scalars().all()
    return request.app.state.templates.TemplateResponse(
        "intelligence/gaps.html",
        {
            "request": request,
            "clusters": clusters,
        },
...
async def gap_detail(request: Request, cluster_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    workspace = request.state.workspace
    cluster = (
        await db.execute(select(GapCluster).where(GapCluster.id == cluster_id, GapCluster.workspace_id == workspace.id))
    ).scalar_one_or_none()
    if not cluster:
    ...
    events = (
        (
            await db.execute(
                select(GapEvent)
                .where(GapEvent.gap_cluster_id == cluster_id)
...
async def sentiment_view(request: Request, range: str = "30d", db: AsyncSession = Depends(get_db)):
    ...
    rows = result.all()
    trends = [
        {"date": row.day.strftime("%b %d"), "avg": round(float(row.avg_sentiment), 2), "count": row.count}
        for row in rows
    ]
...
async def sentiment_segment(
    request: Request, segment: str = "chatbot", days: int = 30, db: AsyncSession = Depends(get_db)
):
    ...
    return request.app.state.templates.TemplateResponse(
        "intelligence/_sentiment_segment.html",
        {
            "request": request,
...
async def trigger_analyze_all(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    convs = (
        await db.execute(
            ...
            .where(Conversation.workspace_id == workspace.id, Conversation.status == "closed")
            .limit(100)
        )
    ).all()
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse(
            ...
            {
                "request": request,
                "message": f"Queued {len(convs)} conversations for analysis",
                "type": "success",
            },
        )
...
async def trigger_sentiment(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse(
            ...
            {
                "request": request,
                "message": "Sentiment trend computation started",
                "type": "success",
            },
        )
...
async def trigger_cluster(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse(
            ...
            {
                "request": request,
                "message": "Gap clustering started",
                "type": "success",
            },
        )
