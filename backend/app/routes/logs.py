...
async def logs_page(
    ...
    if tab == "crawls":
        total = (await db.execute(
            select(func.count(CrawlJob.id)).where(CrawlJob.workspace_id == workspace.id)
        )).scalar_one()
        crawl_jobs = (await db.execute(
            select(CrawlJob).where(CrawlJob.workspace_id == workspace.id)
            .order_by(CrawlJob.created_at.desc()).offset(offset).limit(PAGE_SIZE)
        )).scalars().all()
    elif tab == "documents":
        total = (await db.execute(
        ...
        tasks = (await db.execute(
            select(BackgroundTaskLog).where(BackgroundTaskLog.workspace_id == workspace.id)
            .order_by(BackgroundTaskLog.created_at.desc()).offset(offset).limit(PAGE_SIZE)
        )).scalars().all()

    total_pages = max(1, (total + PAGE_SIZE - 1) // PAGE_SIZE)

    return request.app.state.templates.TemplateResponse("logs/index.html", {
        "request": request, "crawl_jobs": crawl_jobs, "recent_docs": recent_docs, "tasks": tasks,
