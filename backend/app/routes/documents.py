...
async def add_url_source(
    ...
    doc = Document(
        workspace_id=workspace.id, knowledge_base_id=kb.id,
        source_type="url", source_url=url, title=url, status="pending",
    )
    db.add(doc)
    await db.flush()
    await db.commit()
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/document_row.html", {
...
async def add_text_source(
    ...
    doc = Document(
        workspace_id=workspace.id, knowledge_base_id=kb.id,
        source_type="text", raw_content=content, title=title, status="pending",
    )
    db.add(doc)
    await db.flush()
    await db.commit()
...
async def upload_file_source(
    ...
    doc = Document(
        workspace_id=workspace.id, knowledge_base_id=kb.id,
        source_type="file", file_path=file_path, title=file.filename or "Uploaded file", status="pending",
    )
    db.add(doc)
    await db.flush()
    await db.commit()
...
async def reindex_document(request: Request, document_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    if request.headers.get("HX-Request"):
        return request.app.state.templates.TemplateResponse("components/toast.html", {
            "request": request, "message": "Reindexing...", "type": "success",
        })
